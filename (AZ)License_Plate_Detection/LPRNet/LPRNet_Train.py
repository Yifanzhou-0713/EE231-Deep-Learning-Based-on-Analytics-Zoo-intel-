...
def model_creator(config):
    model = LPRNet(class_num=len(CHARS), dropout_rate=0.5)
    model.load_state_dict(torch.load('weights/LPRNet_model_Init.pth', map_location=lambda storage, loc: storage))
    return model
...
def main():
    ...
    parser.add_argument('--cluster_mode',default='local',help='cluster mode of az')
    ...
    orca_estimator.save('weights/model_LPRNet.pth')
