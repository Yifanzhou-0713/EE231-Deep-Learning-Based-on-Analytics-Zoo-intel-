...
if __name__ == '__main__':
    ...
    parser.add_argument("-image", help='image path', default='test/4.jpg', type=str)
    ...
    lprnet.load_state_dict(torch.load('LPRNet/weights/model_LPRNet.pth', map_location=lambda storage, loc: storage))
    # lprnet.eval()
    
    STN = STNet()
    STN.load_state_dict(torch.load('LPRNet/weights/model_STN.pth', map_location=lambda storage, loc: storage))
    # STN.eval()
    ...
    bboxes = create_mtcnn_net(image, args.mini_lp, p_model_path='MTCNN/weights/pnet_Weights.pth', o_model_path='MTCNN/weights/onet_Weights.pth')
    ...
    for i in range(bboxes.shape[0]):
        ...
        data = torch.from_numpy(im).float().unsqueeze(0) # torch.Size([1, 3, 24, 94]) 
        transfer = STN(data)
        ...
        image = cv2ImgAddText(image, labels[0], (x1, y1-50), textColor=(255, 255, 0), textSize=40)
    ...
    cv2.imwrite('test_show2.jpg', image)
